...
<html>
	...
	<body>
		...
		<div class="container-fluid" style="padding-left: 0px;">
			<div class="well">
				<form action="${pageContext.request.contextPath}/selectpackage" method="post">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<tr>					
						...
						<td><c:if test="${pa.getState()=='2'}">运输中</c:if>
							<c:if test="${pa.getState()=='1'}">未发送</c:if>
							<c:if test="${pa.getState()=='3'}">已签收</c:if>
							<c:if test="${pa.getState()=='4'}">已到达</c:if>
						</td>
						<td>${pa.getInformation()}</td>						
						<td>
							<a href="${pageContext.request.contextPath}/courier_package_info?no=${pa.getNo()}">详情</a>
						</td>												
					</tr>				
				</tbody>
				</c:forEach>
